...
/**
 * @brief       Application Bluetooth Low-Energy low-power configuration
 */
struct ble_device_parameter ble_dev_params =
{
    .low_pwr_clk_accuracy = 500,
    .twosc = 3200,
    .rf_tx_power_dbm = 0,
...
/*
 * Random number generator seed value provided by the application.
 * To be used by the BLE stack when calling srand_func().
 */
#if APP_BLE_RAND_SEED_DEFINED
static uint32_t app_rand_seed_val = 1;
#endif

/*
 * Random number generator function provided by the application in place of rand().
 * To be used by the BLE stack when calling rand_func().
 */
#if APP_BLE_RAND_GENERATE_DEFINED
static int (*app_rand_func)(void) = &rand;
#endif

/*
 * RF data to RSSI conversion function provided by the application in place of
 * default_rf_rssi_func(). To be used by the BLE stack when calling Device_RF_RSSI_Convert().
 */
#if APP_BLE_RSSI_CONVERSION_DEFINED
static int8_t (*app_rf_rssi_func)(uint8_t) = &default_rf_rssi_func;
#endif
...
void Device_BLE_Param_Set(struct ble_device_parameter new_params)
{
	memcpy(&ble_dev_params, &new_params, sizeof(struct ble_device_parameter));
}
...
uint8_t Device_BLE_Param_Get(uint8_t param_id, uint8_t *lengthPtr, uint8_t *buf)
{
    ...
    switch (param_id)
    {
        ...
        case PARAM_ID_LE_CODED_PHY_500:
        {
            buf[0] = false;
        }
        break;
...
int rand_func(void)
{
     ...
     * instead of BLE stack using rand().
     */
    return app_rand_func();
#else
    return rand();
#endif
}

void set_app_rand_func(int (*func)(void))
{
#if APP_BLE_RAND_GENERATE_DEFINED
    if (func != NULL)
    {
...
void ecc_get_privateRand(uint8_t *randNum)
{
/* randNum must be filled with a 32-byte value */
#if APP_BLE_RAND_GENERATE_DEFINED
    uint8_t rand_value[32] = APP_PRIVATE_KEY;
    memcpy(randNum, rand_value, 32);
#else
    for (uint8_t i = 0; i < 32; i+= 4)
    {
        co_write32p((randNum + i), rand());
    }
#endif
}
...
{
#if APP_BLE_RSSI_CONVERSION_DEFINED
    return app_rf_rssi_func(rssi_reg);
#else
    return default_rf_rssi_func(rssi_reg);
#endif
}

int8_t default_rf_rssi_func(uint8_t rssi_reg)
{
    return (int8_t)(((29 * rssi_reg) / 100) - 107);
}

void set_app_rf_rssi_func(int8_t (*func)(uint8_t))
{
#if APP_BLE_RSSI_CONVERSION_DEFINED
    if (func != NULL)
    {
        app_rf_rssi_func = func;
    }
#endif
}
