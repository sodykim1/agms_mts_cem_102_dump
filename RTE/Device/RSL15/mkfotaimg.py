...
IMG_HDR_FMT  = struct.Struct("<8L2L")
...
def check_img(imgfile, start_adr, max_size):
    ...
    usagefault_hander, securefault_handler, version_ptr, dscr_ptr) = IMG_HDR_FMT.unpack_from(img)
    ...
    check_bounds(reset_handler, securefault_handler, img_start + img_size, "SecureFault vector invalid")
    ...
    version = IMG_VER_FMT.unpack_from(img, ver_offset)
    print("Version =", version[0])
    assert img_size == size, "Wrong image size in descriptor ({0}/{1})".format(hex(size), hex(img_size))
...
def pad(img, isBoot=False, align=FLASH_SECTOR_SIZE):
    if isBoot:
        img += b'\xFF' * (BOOT_MAX_SIZE - len(img))
    else:
...
def sign(text, sign_key):
    if sign_key:
        ...
        signature = pad(signature, False, CURVE.signature_length)
...
def make(args):
    ...
    fota = pad(sign(fota, None), False)
    fota_size = len(fota)
    print("The start address of the primary app = {} and size = {}".format(hex(FOTA_BASE_ADR), hex(fota_size)))
    ...
    app = sign(app, None)
    print("The start address of the secondary app = {} and size = {}".format(hex(app_start), hex(len(app))))
    
    if args.bootloader:
        boot, ver_offset, boot_id = check_img(args.bootloader, BOOT_BASE_ADR, BOOT_MAX_SIZE)
        boot = embed_devid(boot, ver_offset, args.devid)
        boot = pad(sign(boot, None), True)
        return boot + fota + app
    else:
        return fota + app
...
if __name__ == "__main__":
    ...
    global BOOT_MAX_SIZE
    global FOTA_BASE_ADR
    global FOTA_MAX_SIZE
    global APP_START_ADDR
   
    BOOT_MAX_SIZE = 32 * 1024

    FOTA_BASE_ADR = BOOT_BASE_ADR + BOOT_MAX_SIZE
    FOTA_MAX_SIZE = (FLASH_SIZE - BOOT_MAX_SIZE)
    ...
    parser = argparse.ArgumentParser(description='Build tool for FOTA image files.')
    ...
    parser.add_argument('-s', '--secure', dest="secure", metavar='SecureBoot', type=secure_addr,
                        help="The size of secure bootloader for the secure FOTA (default: non secure)")
    ...
    parser.add_argument('-n', '--name', dest="name", metavar='NAME', type=utf8str,
                        help="advertised name to embed in the image (default: 'RSL FOTA')")
    ...
    parser.add_argument('-b', '--bootloader', metavar='BOOT-IMG', type=argparse.FileType('rb'),
                        help="Bootloader sub-image (.bin file)")
    ...
    if args.secure:
        BOOT_MAX_SIZE = args.secure
        FOTA_BASE_ADR = BOOT_BASE_ADR + BOOT_MAX_SIZE
        FOTA_MAX_SIZE = (FLASH_SIZE - BOOT_MAX_SIZE)
    print("The size of the bootloader=", hex(BOOT_MAX_SIZE))

    if args.bootloader:
        with args.bootloader, args.fota, args.app, args.out:
